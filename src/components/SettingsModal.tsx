...
export default function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  // tab 状态中，"api" 现在代表云端模型设置，"local" 代表本地模型设置
  ...
  const checkOllamaStatus = async () => {
    ...
    try {
      ...
      if (data.status === "online") {
        ...
        setOllamaStatusMsg(data.message || "服务离线 (Offline)");
      }
    } catch (err) {
      setOllamaStatus("offline");
    ...
    try {
      ...
      if (data.status === "success" && data.models) {
        ...
        if (fetchedModels.length === 0) {
          alert("Ollama 连接成功，但在本地未发现已下载的模型。请在终端执行 'ollama run <model>' 下载模型。");
          return;
        }
        ...
        // 寻找是否存在 Ollama 专属配置块，存在则更新模型，不存在则追加配置块
        const ollamaIdx = currentConfigs.findIndex(c => c.providerName.toLowerCase() === "ollama");
        ...
        // 通知 App.tsx 主页进行模型热更新
        window.dispatchEvent(new Event("tangerine_api_settings_changed"));

        alert(`已成功同步 ${fetchedModels.length} 个本地模型并导入到下方本地管理区！`);
      } else {
        alert("未能获取到有效的本地模型。");
      }
    } catch (err: any) {
  ...
  // 分流配置：云端模型设置过滤掉 ollama，避免互相抢占位置
  const cloudConfigs = configs.filter(c => c.providerName.toLowerCase() !== "ollama");
  
  // 查找单独在本地设置中维护展示的 ollama 配置块
  const ollamaConfig = configs.find(c => c.providerName.toLowerCase() === "ollama");
      ...
      <div className="w-full h-full flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col h-full bg-[#1e1e1f] overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-thin">
            ...
            {/* TAB 1: 云端模型设置（只渲染排除 ollama 后的云端模型列表） */}
            {activeTab === "api" && (
              <>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mb-4">
                    使用运行在您本机的 Ollama 服务，实现安全、隐私、无网络流量消耗的完全离线 AI 交互。
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-white/5 bg-[#141415] p-6 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-xs text-gray-400 font-semibold">Ollama 运行状态</div>
                ...
                <div className="rounded-xl border border-white/5 bg-[#141415] p-6 space-y-4">
                  <div>
                    <h4 className="text-xs font-bold text-white mb-1">自动检索本地模型</h4>
                    <p className="text-[11px] text-gray-500 leading-relaxed">
                      智能读取您本地已经下载过的 Ollama 模型，并将模型名称自动同步并展示在下方的配置管理面板中。
                    </p>
                  </div>
                  ...
                  >
                    {isSyncing ? "正在同步模型..." : "查询当前 Ollama 模型并写入主页"}
                  </button>
                </div>
                  ...
                  <div className="p-6 rounded-xl border border-white/5 bg-[#141415] shadow-lg hover:border-white/10 transition-all mt-6">
                    <div className="flex items-center justify-between mb-5 pb-3 border-b border-white/5">
                      <span className="text-xs font-bold text-emerald-400 tracking-wider uppercase font-mono flex items-center gap-1.5">
                        <Cpu size={14} />
                        本地模型管理区 ({ollamaConfig.providerName})
                      </span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          ...
                          <span>{savedStatus[ollamaConfig.id] ? "保存成功" : "保存本配置"}</span>
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="border border-white/5 rounded-lg bg-[#19191a] overflow-hidden mt-4">
                        <button
                          ...
                          className="w-full flex items-center justify-between px-4 py-3 text-xs text-gray-200 hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
                        >
                          <span className="font-semibold">本地模型列表 ({ollamaConfig.models.length})</span>
